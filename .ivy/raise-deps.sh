...
for pkg in webviews/*/package.json package.json; do
  update_version "$pkg"
done
mvn --batch-mode versions:set-property versions:commit -Dproperty=openapi.version -DnewVersion="$VERSION" -DallowSnapshots=true
