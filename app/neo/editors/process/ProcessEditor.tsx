...
export const ProcessEditor = ({ id, project, path, name }: Editor) => {
  ...
  const editorUrl = useHref(
    `/process-editor/index.html?server=${`${baseUrl()}${ws?.baseUrl}`}&app=${project.app}&project=${project.project}&file=${path}${PROCESS_EDITOR_SUFFIX}&readonly=${project.isIar ?? false}`
  );
