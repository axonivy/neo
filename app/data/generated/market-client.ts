...
export const getFindBestMatchVersionUrl = (id: string, version: string, params?: FindBestMatchVersionParams) => {
  const normalizedParams = new URLSearchParams();
  ...
  return stringifiedParams.length > 0
    ? `/api/product-details/${id}/${version}/best-match-version?${stringifiedParams}`
    : `/api/product-details/${id}/${version}/best-match-version`;
};

export const findBestMatchVersion = async (
  id: string,
  version: string,
  params?: FindBestMatchVersionParams,
  options?: RequestInit
): Promise<findBestMatchVersionResponse> => {
...
export const getGetTestReportUrl = (productId: string, workflow: 'CI' | 'DEV' | 'E2E') => {
