...
export const useImportProjectsIntoWs = () => {
  ...
  const importProjects = async (id: string, file: Blob, dependentProject?: ProjectIdentifier) => {
    ...
    const res = await importProjectsReq(id, { file, dependentProject: blob, doConvert: 'true' });
    if (ok(res)) {
      return;
    }
...
export const useDeployWorkspace = () => {
  ...
  const deployWorkspace = async (params: DeployParams) => {
    ...
    const res = await deploy('default', params.applicationName, 'released', { fileToDeploy }, { headers: reqHeaders });
    if (ok(res)) {
  ...
  return {
    installProduct: (id: string, json: string, project?: ProjectIdentifier) => {
      ...
      toast.promise(() => installResult, {
        ...
        success: res => t('toast.workspace.marketProductInstalled', { projects: res.installedProjects.map(p => p.project).join(', ') }),
        error: e => e.message
      });
