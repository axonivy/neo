...
const openJumpProcess = async (page: Page) => {
  const neo = await Neo.openWorkspace(page, `processes/${APP}/${TEST_PROJECT}/process/jump`);
...
const openHdProcess = async (page: Page) => {
  const neo = await Neo.openWorkspace(page, `processes/${APP}/${TEST_PROJECT}/dialog/neo/test/project/EnterProduct/EnterProductProcess`);
...
const openSubProcess = async (page: Page) => {
  const neo = await Neo.openWorkspace(page, `processes/${APP}/${TEST_PROJECT}/process/subproc`);
