...
test('home', async ({ page }) => {
  ...
  const overview = new Overview(page);
  await expect(overview.title.first()).toHaveText(/Welcome to your workspace/);
  await overview.expectCardsCountGreaterThan(0);
...
test('project', async ({ page }) => {
  ...
  await expect(overview.title.first()).toHaveText(/Project details/);
...
test('market', async ({ page }) => {
  ...
  await expect(market.title.first()).toHaveText(/Axon Ivy Market/);
